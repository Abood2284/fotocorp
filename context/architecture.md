...
- Public Typesense search: `GET /api/v1/search/assets` through the public catalog Hono route module. It queries Typesense alias `public_assets_current` for public search results, category/event/city/source facets, counts, and page pagination. It returns both the PR-5 response fields (`total`, `perPage`, `totalPages`, `timing`) and compatibility fields (`totalCount`, `limit`, `meta`). Anonymous public search responses use `Cache-Control: public, max-age=30, s-maxage=120, stale-while-revalidate=300`. `/search` now uses the same-origin `/api/public/search/assets` BFF path directly with TanStack Query client caching; `/api/v1/assets` remains a compatibility/listing route, not the homepage/search query path.
