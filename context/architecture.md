...
- Caricature uploads live in private `caricature_assets` (+ R2 `fotocorp-caricature-originals`). Contributor/staff upload-shell sets `created_by_contributor_id` so contributor dashboard analytics and `/contributor/uploads` can attribute caricatures alongside editorial `image_assets` / upload batches.
...
- Staff auth API (Hono): `POST /api/v1/staff/auth/login`, `POST /api/v1/staff/auth/logout`, `GET /api/v1/staff/auth/me` (`apps/api/src/routes/staff/auth/route.ts`). Sales-led inquiry + entitlement routes: `GET/POST /api/v1/staff/access-inquiries*`, `PATCH /api/v1/staff/access-inquiries/:inquiryId/contributor-allowed-upload-types`, `PATCH/POST /api/v1/staff/subscriber-entitlements/*` (activate + suspend) (`apps/api/src/routes/staff/access-inquiries/`). Staff audit read: `GET /api/v1/staff/audit-logs` (`apps/api/src/routes/staff/audit-logs/route.ts`) — `SUPER_ADMIN` only; unified cursor-paginated feed across `staff_audit_logs`, `asset_admin_audit_logs`, and `admin_user_audit_logs` with sanitized metadata. Staff productivity read: `GET /api/v1/staff/productivity` (+ `/:staffMemberId`, `/:staffMemberId/activity`, `/:staffMemberId/export`) (`apps/api/src/routes/staff/productivity/route.ts`) — `SUPER_ADMIN` only; primary KPI is unique assets touched, with per-field save/unique breakdown from `asset_admin_audit_logs` (catalog/captions/upload-wizard) plus contributor-upload metadata/approval/rejection counts from `staff_audit_logs`; detail includes `activityByDay`, activity ledger, and CSV export. Staff UI: `/staff/audit`, `/staff/team-performance`, `/staff/team-performance/[staffMemberId]`. Write-side audit foundation remains `staff_audit_logs` for staff-domain events (e.g. `STAFF_LOGIN_SUCCESS`, `STAFF_LOGIN_FAILED`, `STAFF_LOGOUT`, contributor-upload review actions), while asset editorial before/after deltas remain in `asset_admin_audit_logs` (including staff upload-wizard metadata saves). Bootstrap: `pnpm --dir apps/api staff:bootstrap` with `STAFF_BOOTSTRAP_*` vars documented in `apps/api/.dev.vars.example`.
- Staff workspace RBAC (`apps/web/src/lib/staff/staff-route-access.ts`): **`CAPTION_WRITER`** accesses `/staff/contributor-uploads` (full pre-publish workflow), `/staff/captions`, `/staff/catalog`, `/staff/homepage-hero`, and `/staff/events`; only **`SUPER_ADMIN`** and **`CAPTION_WRITER`** may access contributor uploads. **`CATALOG_MANAGER`** and **`REVIEWER`** no longer access contributor uploads (catalog/homepage-hero remain for catalog manager). Event purge remains **`SUPER_ADMIN`** only at the API layer. `/staff/team-performance` is `SUPER_ADMIN` only. Contributor upload volume for admins is aggregateable from `contributor_upload_batches` / `contributor_upload_items` by `contributor_id`; staff caption/upload productivity is aggregateable from existing audit rows without duplicating revision tables.
...
- Main-app photographer roles remain part of the Better Auth/app-user model, but the dedicated contributor portal uses separate `contributor_accounts` and `contributor_sessions`; these sessions do not grant staff or subscriber access. Contributor accounts may carry `portal_role` (`STANDARD` | `PORTAL_ADMIN`); `PORTAL_ADMIN` can list all contributors and must choose a target photographer when creating events for someone else. Contributors also carry `allowed_upload_types` (`EDITORIAL` / `CARICATURE`, multi) on `contributors`; staff assign this on contributor application detail (approve + post-approve PATCH). Contributor upload UI/API enforce the set; staff upload wizard stays unrestricted. Read-only `GET /api/v1/contributor/catalog/asset-categories` backs event category pickers. Contributor dashboard/uploads show a header **Need help?** card for matching upload guides. Allowlisted articles are readable at `/contributor/help/[slug]` in DRAFT or PUBLISHED status (`GET /api/v1/contributor/help/articles/:slug` + media `GET /api/v1/contributor/help/media/:mediaId`); the staff Help Center stays staff-session-only.
