...
| Jobs CLI | Node.js package in `apps/jobs` | Background image publish worker (Sharp, R2, DB). **Not** a Cloudflare Worker; no `fetch` entry. Invoked via `tsx` CLI scripts. **PR-16E:** deployable as a **private Docker** service on a VPS (see `docker-compose.jobs.yml`, `apps/jobs/Dockerfile`, and [`jobs-direct-vps-deployment-runbook.md`](../docs/db-revamp/jobs-direct-vps-deployment-runbook.md)); no inbound HTTP port. **PR-16F:** real Neon polling for `image_publish_jobs` via native `pg` with `FOR UPDATE SKIP LOCKED` claim; gated by safety flag `IMAGE_PUBLISH_PROCESSING_ENABLED` (default **`false`**). With the default the worker only counts queued jobs; with `true` it runs a placeholder lifecycle (job + items → `FAILED` with `failure_code = PROCESSING_NOT_IMPLEMENTED`) and never mutates `image_assets`. Real Sharp/R2 processing is still owned by the API CLI `pnpm --dir apps/api media:process-image-publish-jobs`. |
...
| Media processing | Sharp (Node-capable contexts) | Derivative generation uses native Sharp in Node-capable scripts. **`apps/api`** still hosts today’s processor CLI (`pnpm --dir apps/api media:process-image-publish-jobs`). **`apps/jobs`** is the Node-oriented home for future publish-worker Sharp work; do not import native Sharp into the Worker runtime. |
...
- `apps/jobs` is a **Node.js** worker package (Sharp; PR-16F real Neon publish-job polling). It is **Dockerized for private VPS deployment** (no published ports, no browser access). **`apps/web` must never call `apps/jobs` directly**; staff flows use **`apps/api`** to persist work in Neon, and the worker consumes that work outbound-only (Neon, R2; optional queues later). PR-16F wired the worker to read `image_publish_jobs` directly via native `pg`; the safety flag `IMAGE_PUBLISH_PROCESSING_ENABLED` (default `false`) determines whether the worker only counts queued jobs or runs the placeholder claim lifecycle. Real image processing (Sharp + R2 promotion to public) is still owned by `pnpm --dir apps/api media:process-image-publish-jobs`; a follow-up PR will move that processor into `apps/jobs`.
