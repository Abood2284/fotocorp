...
- Public Typesense search: `GET /api/v1/search/assets` through the public catalog Hono route module. It queries Typesense alias `public_assets_current` for public search results, category/event/city/source facets, counts, and page pagination. It returns both the PR-5 response fields (`total`, `perPage`, `totalPages`, `timing`) and compatibility fields (`totalCount`, `limit`, `meta`). It does not replace `/api/v1/assets` or `/api/v1/assets/filters`; the frontend `/search` route selects it only when `NEXT_PUBLIC_USE_TYPESENSE_SEARCH=true`.
