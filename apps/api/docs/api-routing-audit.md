...
- **Staff Help Center contextual links (PR 5):** Added `help_contextual_links` table via migrations `0058_closed_blonde_phantom.sql` (fresh installs) and repair migration `0061_help_contextual_links.sql` (idempotent; fixes DBs where 0058 was marked applied but rolled back because it incorrectly duplicated 0057 media columns). Staff read `GET /api/v1/staff/help/contextual-links`; manager CRUD under `/api/v1/staff/help/manage/contextual-links`. Seed: `pnpm --dir apps/api db:seed:help-contextual-links`.

- **Staff Help Center media upload (PR 4):** Private R2 bucket `fotocorp-help-center` via `MEDIA_HELP_CENTER_BUCKET` / `R2_HELP_CENTER_BUCKET`. Storage prefix `help-media/articles/{articleId}/{mediaId}/{safeFilename}`. Migration `0057_help_article_media_upload_status.sql` adds upload lifecycle columns on `help_article_media`. Routes: manager `POST .../media/upload-intent`, `PUT .../media/:mediaId/upload` (same-origin BFF bytes → R2; no browser CORS on bucket), `POST .../media/:mediaId/confirm`, `PATCH .../media/:mediaId`, `DELETE .../media/:mediaId`, `POST .../media/reorder`; staff read `GET /api/v1/staff/help/media/:mediaId` (range-aware, role/article visibility enforced). Web BFF binary route `/api/staff/help/media/:mediaId` and upload proxy `/api/staff/help/articles/:articleId/media/:mediaId/upload`; catch-all supports `DELETE`. Allowed types: PNG/JPG/WEBP (10 MB), MP4/WEBM (100 MB).

- **Staff Help Center content management UI (PR 3):** Web routes `/staff/help/manage`, `/staff/help/manage/new`, `/staff/help/manage/[articleId]/edit`, `/staff/help/manage/categories`, `/staff/help/manage/tags` for `SUPER_ADMIN` + `CATALOG_MANAGER` only. Article create/edit/publish/archive, markdown preview, category/tag management, management list filters via URL params. Proxies through existing `/api/staff/[...path]` BFF. Added manager read routes `GET /api/v1/staff/help/manage/categories` and `GET /api/v1/staff/help/manage/articles/:articleId`. **Next:** PR 5 contextual help links.

- **Staff Help Center read UI (PR 2):** Web routes `/staff/help` and `/staff/help/[slug]` for authenticated staff. Search/filter via URL params (`q`, `category`, `tag`); article cards; markdown detail; related articles; helpful feedback. Proxies through existing `/api/staff/[...path]` BFF to PR 1 help routes. Nav item **Help** (all staff roles). No admin editing UI in this PR.

- **Staff Help Center foundation (PR 1):** Added help center schema (`help_categories`, `help_articles`, `help_tags`, `help_article_tags`, `help_article_media`, `help_article_related`, `help_article_feedback`, `help_article_views`) via migration `0056_past_enchantress.sql`. Staff-only routes under `/api/v1/staff/help/*` (read for all authenticated staff; create/update for `SUPER_ADMIN` + `CATALOG_MANAGER`). Postgres `ILIKE` search across title/summary/body and tag slug/name; article detail records views. Idempotent seed: `pnpm --dir apps/api db:seed:help-center`. Route ownership: `apps/api/src/routes/staff/help/route.ts`. **Next:** PR 3 Help Content Management UI.

## Incremental update (2026-06-16)

- **Caricature automated publish pipeline (PR 11b):** `POST /api/v1/internal/admin/caricature-assets/:assetId/approve` enqueues `caricature_preview_jobs`, queues blurred derivatives, and pings `JOBS_DRAIN_WEBHOOK_URL` (same editorial wake). `POST .../:assetId/reject` marks `REJECTED`. Jobs drain (`apps/jobs`) claims caricature preview jobs, runs Sharp blur, optionally publishes (`PUBLISHED`/`PUBLIC`), and calls `POST /api/v1/internal/search/typesense/sync-caricature`. Staff review UI: `/staff/caricatures`. Manual CLI scripts remain for operator backfill only.
- **Caricature publish flow + public detail (PR 11):** `GET /api/v1/caricatures/:assetId` serves published/public caricature detail with blurred preview URLs only. Staff original stream: `GET /api/v1/internal/admin/caricature-assets/:assetId/original`. Web: `/caricatures/[id]`, search grid links, staff wizard review panel + BFF proxies for original/generate-previews.
- **Caricature blurred preview generation (PR 10):** `POST /api/v1/internal/admin/caricature-assets/:assetId/generate-previews` queues preview work and pings the jobs drain webhook (preview-only, no publish). Node script `pnpm --dir apps/api media:generate-caricature-previews` remains for operator backfill.
- **Caricature original upload (PR 9):** Private R2 bucket `fotocorp-caricature-originals` via Worker binding `MEDIA_CARICATURE_ORIGINALS_BUCKET` and S3 presign vars `R2_CARICATURE_ORIGINALS_BUCKET`. Routes: `POST /api/v1/internal/admin/caricature-assets/upload-shell`, `POST .../:assetId/original/presign`, `POST .../:assetId/original/complete` (mirrored on contributor `/api/v1/contributor/caricatures/*`). `GET /api/v1/contributor/caricatures/:assetId/original` streams the private original for authenticated contributor upload-wizard metadata preview (proxied at `/api/contributor/caricatures/:assetId/original`). Upload wizard step 3 presigns browser PUT, verifies object, and sets `original_bucket` / `original_object_key` on `caricature_assets`. Staff BFF: `/api/staff/upload-wizard/caricatures/upload-shell`, `.../original/presign`, `.../original/complete`, `GET .../:assetId/original`. Upload-shell accepts optional `contributorId` and stores `caricature_assets.created_by_contributor_id` so contributor dashboard/uploads can attribute caricatures.
- **Caricature staff metadata CRUD + upload wizard (PR 8):** Internal routes `GET|POST /api/v1/internal/admin/caricature-assets`, `GET|PATCH|DELETE .../:assetId` (DELETE soft-deletes published assets and removes Typesense docs), with language/visible-text validation and publish blocked until original file exists. Contributor routes `GET /api/v1/contributor/caricatures` (owned list), `GET /api/v1/contributor/caricatures/categories`, `POST|GET|PATCH /api/v1/contributor/caricatures/:assetId` (ownership-scoped). Caricature is enabled in step 1 of the shared staff/contributor upload wizard; editorial keeps Event → Upload → Metadata, caricature uses Details → single-file Upload → caricature Metadata. Staff BFF: `/api/staff/upload-wizard/caricature-categories`, `/api/staff/upload-wizard/caricatures`, plus review BFF `GET|PATCH|DELETE /api/staff/caricatures/:assetId`. Staff review UI defaults to All and supports edit (Draft/Pending) + delete (Published).
...
- **`SUPER_ADMIN` staff productivity read:** `GET /api/v1/staff/productivity` — primary KPI is **unique assets touched** (any tracked metadata field change), plus save counts and per-field breakdown (caption / who-is-in-picture / keywords / headline / description) from `asset_admin_audit_logs` and contributor-upload `staff_audit_logs` (`changedFields`). Also returns `activityByDay` and metric `definitions`. Upload approvals/rejections remain from contributor-upload audit counts. Detail: `GET /api/v1/staff/productivity/:staffMemberId`, activity log `GET .../:staffMemberId/activity` (cursor), CSV `GET .../:staffMemberId/export`. Query params `from`, `to`. Staff upload-wizard metadata PATCH now writes `asset_admin_audit_logs` with actor headers. Route ownership: `apps/api/src/routes/staff/productivity/route.ts`. Web BFF proxies via `/api/staff/productivity`; staff UI at `/staff/team-performance` and `/staff/team-performance/[staffMemberId]`.
...
- `POST /api/v1/auth/sign-up` remains owned by `apps/api/src/routes/platform-auth/route.ts`; after successful user/account/inquiry creation it sends `CUSTOMER_ACCESS_REQUEST_RECEIVED` to the registrant and `STAFF_NEW_ACCESS_INQUIRY` to hardcoded staff recipients (`STAFF_INQUIRY_NOTIFY_EMAILS`, one message with all addresses in `To`) with full lead details (contact, interest, geo/IP; no staff UI link). Both use `apps/api/src/lib/email/email-service.ts`. Email failure is logged and does not fail registration.
- `POST /api/v1/public/contributor-applications` remains owned by `apps/api/src/routes/public/contributor-applications/route.ts`; after successful application inquiry creation it sends `CONTRIBUTOR_APPLICATION_RECEIVED` when the applicant supplied an email, and always sends `STAFF_NEW_CONTRIBUTOR_APPLICATION` to the same hardcoded staff recipients with full lead details. Email failure is logged and does not fail application submission.
...
- Web BFF `GET`/`HEAD` `/api/public/events/latest`, `/api/public/caricatures/latest`, `/api/public/search/assets`, `/api/public/search/caricatures`, and `/api/public/assets/:assetId` remain owned by `apps/web/src/app/api/public/[...path]/route.ts`; these public paths explicitly preserve/set public cache headers instead of falling back to `private, no-store`.
...
- `GET /api/v1/public/caricatures/latest` serves cursor-paginated published caricatures from `caricature_assets` + ready `BLURRED_CARD` derivatives (default no recency window; optional `windowDays`, `limit` default 25, optional cursor); returns blurred CDN preview URLs plus card metadata (`description`, `language`, `hasVisibleText`, `hasTranslation`, `depictedSubjects`) — no visible text transcript on list responses. Cache matches events/latest (`public, max-age=60, s-maxage=300, stale-while-revalidate=3600`). Purged on caricature Typesense sync after publish.
...
| `GET` | `/api/v1/search/caricatures` | `searchTypesenseCaricatures` + `listPublicCaricatures` fallback | `apps/api/src/routes/public/catalog-routes.ts` + `apps/api/src/lib/search/typesense-caricatures.ts` + `apps/api/src/lib/caricatures/public-caricature-assets.ts` | Typesense-backed caricature segment search (`status:=PUBLISHED && visibility:=PUBLIC`). Facets: categories, languages, credits, hasVisibleText, depictedSubjects; `meta.popularSortAvailable` gates Popular sort in UI. Postgres fallback computes facet aggregates when `includeFacets=true`. BFF: `/api/public/search/caricatures`. Wired by `/search?segment=caricature`. |
...
| `GET` | `/api/v1/public/caricatures/latest` | `fetchPublicLatestCaricaturesRows` | `apps/api/src/routes/public/homepage-routes.ts` + `apps/api/src/lib/caricatures/public-homepage-caricatures.ts` | PR-2 uses `PUBLIC_READ_HYPERDRIVE`. Cursor-paginated homepage caricature catalog from `caricature_assets` (`PUBLISHED`/`PUBLIC`, ready `BLURRED_CARD` preview). No recency window unless `windowDays` is set. Default `limit=25`. Keyset on `published_at desc, id desc`. Returns blurred CDN `public_url` plus card metadata (`description`, `language`, `hasVisibleText`, `hasTranslation`, `depictedSubjects`). BFF: `/api/public/caricatures/latest`. Homepage Caricature tab + nav Show all (`/?tab=caricature#homepage-categories`). Cache purge on `sync-caricature`. |
...
| `POST` | `/api/v1/internal/admin/assets/:assetId/generate-previews` | `queueImagePreviewRegenerationService` | `apps/api/src/routes/internal/admin/route.ts` | Queues `image_preview_regeneration_jobs` and pings jobs drain webhook. Preview-only (no auto-publish). |
| `GET` | `/api/v1/internal/admin/jobs-pipeline/snapshot` | `jobsPipelineSnapshotService` | `apps/api/src/routes/internal/admin/route.ts` | Read-only snapshot of catalog regen, image publish item, and caricature preview queues plus recent derivative activity. Optional jobs `/health` probe for drain status. |
| `POST` | `/api/v1/internal/admin/jobs-pipeline/wake` | `jobsPipelineWakeService` | `apps/api/src/routes/internal/admin/route.ts` | POSTs to VPS `JOBS_DRAIN_WEBHOOK_URL` to start a jobs drain (same wake as contributor approve). Returns structured accept/busy/unreachable result. |
...
| `GET` | `/api/v1/internal/admin/users` | `internalAdminUsersRoute` | `apps/api/src/routes/internalAdminCatalog.ts` | Explicit method guard in `index.ts`. List includes `username`; search matches email, display name, username, and id. |
| `GET` | `/api/v1/internal/admin/users/:authUserId/downloads` | `adminUserDownloadsService` | `apps/api/src/routes/internal/admin/route.ts` | Staff read of a user's `image_download_logs` with optional `from`/`to` date filters, cursor pagination, preview URLs, and total count. |
