...
- `GET /api/v1/public/caricatures/latest` serves cursor-paginated latest published caricatures from `caricature_assets` + ready `BLURRED_CARD` derivatives (`windowDays`, `limit`, optional cursor); returns blurred CDN preview URLs plus card metadata (`description`, `language`, `hasVisibleText`, `hasTranslation`, `depictedSubjects`) — no visible text transcript on list responses. Cache matches events/latest (`public, max-age=60, s-maxage=300, stale-while-revalidate=3600`). Purged on caricature Typesense sync after publish.
...
| `GET` | `/api/v1/public/caricatures/latest` | `fetchPublicLatestCaricaturesRows` | `apps/api/src/routes/public/homepage-routes.ts` + `apps/api/src/lib/caricatures/public-homepage-caricatures.ts` | PR-2 uses `PUBLIC_READ_HYPERDRIVE`. Cursor-paginated homepage latest caricatures from `caricature_assets` (`PUBLISHED`/`PUBLIC`, ready `BLURRED_CARD` preview). Keyset on `published_at desc, id desc`. Returns blurred CDN `public_url` plus card metadata (`description`, `language`, `hasVisibleText`, `hasTranslation`, `depictedSubjects`). BFF: `/api/public/caricatures/latest`. Homepage Caricature tab + nav Latest (`/?tab=caricature#homepage-categories`). Cache purge on `sync-caricature`. |
