...
| `GET` | `/api/v1/public/homepage/hero-set` | `getPublicHomepageHeroSet` | `apps/api/src/routes/public/homepage-routes.ts` | Random 9 from staff-curated `public_homepage_hero_pool_items` (requires exactly 25 eligible rows). Cache `public, max-age=0, s-maxage=30, stale-while-revalidate=60`. Staff save at `/staff/homepage-hero` is the write path. |
