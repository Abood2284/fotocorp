...
- **Staff Help Center contextual links (PR 5):** Added `help_contextual_links` table via migration `0058_closed_blonde_phantom.sql`. Maps workflow `context_key` values (for example `staff.assets.upload`) to `help_articles` with optional label/description, placement (`PAGE_HEADER` | `SIDEBAR_CARD` | `INLINE_PANEL`), display order, and `is_active`. Staff read: `GET /api/v1/staff/help/contextual-links?contextKey=...` returns only active links whose linked article is `PUBLISHED` and visible to the caller's role. Manager CRUD: `GET|POST /api/v1/staff/help/manage/contextual-links`, `PATCH|DELETE .../:linkId` (`DELETE` soft-deactivates). Idempotent seed: `pnpm --dir apps/api db:seed:help-contextual-links` (slug-based, skips missing articles). Web: reusable `ContextualHelpPanel` on staff workflow pages; management UI `/staff/help/manage/contextual-links`. **Next:** PR 6 optional help analytics/quality improvements.

- **Staff Help Center media upload (PR 4):** Private R2 bucket `fotocorp-help-center` via `MEDIA_HELP_CENTER_BUCKET` / `R2_HELP_CENTER_BUCKET`. Storage prefix `help-media/articles/{articleId}/{mediaId}/{safeFilename}`. Migration `0057_help_article_media_upload_status.sql` adds upload lifecycle columns on `help_article_media`. Routes: manager `POST .../media/upload-intent`, `POST .../media/:mediaId/confirm`, `PATCH .../media/:mediaId`, `DELETE .../media/:mediaId`, `POST .../media/reorder`; staff read `GET /api/v1/staff/help/media/:mediaId` (range-aware, role/article visibility enforced). Web BFF binary route `/api/staff/help/media/:mediaId`; catch-all now supports `DELETE`. Allowed types: PNG/JPG/WEBP (10 MB), MP4/WEBM (100 MB).
