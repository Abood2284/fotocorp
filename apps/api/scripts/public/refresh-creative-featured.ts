...
async function main() {
  ...
  try {
    ...
    console.log(
      `[creative-featured] period=${options.period} limit=${options.limit} selected=${candidates.length} dryRun=${options.dryRun}`,
    )
    console.log(
      "[creative-featured] eligibility=ACTIVE+PUBLIC image assets with ready CARD and DETAIL previews; category name 'creative' is preferred when present, otherwise newest public-ready assets fill the feed.",
    )
    ...
    console.log(`[creative-featured] refreshed period=${options.period} active=${candidates.length}`)
  } finally {
    await pool.end()
  }
}
...
async function refreshPeriod(pool: pg.Pool, period: string, candidates: CandidateRow[]) {
  ...
  try {
    ...
    await client.query(
      `
        delete from public_creative_featured_items
        where period_key = $1
          and status = 'INACTIVE'
      `,
      [period],
    ...
    await client.query(
      `
        update public_creative_featured_items
        set status = 'INACTIVE',
            rank = rank + 100000,
            updated_at = now()
        where period_key = $1
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      await client.query(
        `
          insert into public_creative_featured_items (asset_id, period_key, rank, status)
          values ($1, $2, $3, 'ACTIVE')
...
function printHelp() {
  console.log(`Refresh precomputed Creative featured feed.
