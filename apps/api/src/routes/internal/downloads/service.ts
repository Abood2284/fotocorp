...
import {
  normalizeDownloadRequestAudit,
  type NormalizedDownloadRequestAudit,
} from "../../../lib/downloads/download-request-audit"

const downloadRequestAuditSchema = z.object({
  ipAddress: z.string().trim().max(256).nullable().optional(),
  ipHash: z.string().trim().max(128).nullable().optional(),
  country: z.string().trim().max(2).nullable().optional(),
  city: z.string().trim().max(256).nullable().optional(),
  region: z.string().trim().max(256).nullable().optional(),
  regionCode: z.string().trim().max(32).nullable().optional(),
  cfRay: z.string().trim().max(256).nullable().optional(),
...
const downloadRequestSchema = z.object({
  ...
  requestAudit: downloadRequestAuditSchema.optional(),
})
...
interface DownloadAssetRow { id: string; fotokey: string | null; r2_original_key: string | null; original_filename: string | null; original_ext: string | null; r2_exists: boolean; status: string; visibility: string; media_type: string }
...
interface ValidatedSubscriberDownload {
  ...
  size: DownloadSize
  audit: NormalizedDownloadRequestAudit
  authUserId: string
...
async function validateSubscriberDownloadRequest(env: Env, assetId: string, request: Request, options: { recordFailures: boolean }): Promise<ValidatedSubscriberDownload> {
  ...
  const { authUserId, size, userAgent, requestIp, requestAudit } = parsed.data
  ...
  if (!asset) {
    if (options.recordFailures) await writeDownloadFailure(db, { assetId, profile, size, failureCode: "ASSET_NOT_FOUND", audit })
  ...
  if (!isAssetDownloadable(asset)) {
    if (options.recordFailures) await writeDownloadFailure(db, { assetId: asset.id, profile, size, failureCode: "ASSET_NOT_DOWNLOADABLE", audit })
  ...
  try {
    ...
    if (error instanceof AppError && options.recordFailures) {
      await writeDownloadFailure(db, { assetId: asset.id, profile, size, failureCode: error.code, audit })
    }
  ...
  return { db, profile, asset, size, audit, authUserId, entitlementId }
}
...
export async function internalSubscriberAssetDownloadService(request: Request, env: Env, assetIdRaw: string): Promise<Response> {
  ...
  const { profile, asset, size, audit, authUserId, entitlementId } = validated
  ...
  if (!quota) {
    ...
    await writeDownloadFailure(db, { assetId: asset.id, profile, size, failureCode: "DOWNLOAD_LIMIT_EXCEEDED", audit })
  ...
  if (!object?.body) {
    await writeDownloadFailure(db, { assetId: asset.id, profile, size, failureCode: "SOURCE_FILE_NOT_FOUND", audit })
    throw new AppError(409, "SOURCE_FILE_NOT_FOUND", "This asset is not available for clean download.")
  }
  const logId = await writeDownloadStarted(db, { asset, profile, quota, size, object, audit })
...
async function writeDownloadStarted(db: DrizzleClient, values: { asset: DownloadAssetRow; profile: DownloadProfileRow; quota: QuotaUpdateRow; size: DownloadSize; object: Awaited<ReturnType<typeof getR2Object>>; audit: NormalizedDownloadRequestAudit }): Promise<string | null> {
  try {
    const rows = await executeRows<{ id: string }>(
      ...
      sql`
        ...
        values (
          ...
          ${values.audit.userAgent},
          ${values.audit.ipHash},
          ${values.audit.ipAddress},
          ${values.audit.country},
          ${values.audit.city},
          ${values.audit.region},
          ${values.audit.regionCode},
          ${values.audit.cfRay},
          'APPLICATION'
        )
        returning id
...
async function writeDownloadFailure(db: DrizzleClient, values: { assetId?: string; profile: DownloadProfileRow; size: DownloadSize; failureCode: string; audit: NormalizedDownloadRequestAudit }): Promise<void> {
  try {
    await db.execute(sql`
      insert into image_download_logs (
        image_asset_id,
