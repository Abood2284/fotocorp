...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const sessionCookie = cookieStore.get(FOTOCORP_PLATFORM_SESSION_COOKIE)
  if (!sessionCookie?.value) {
    return Response.json({ error: { code: "CONTRIBUTOR_UNAUTHORIZED" } }, { status: 401 })
  }

  const upstreamUrl = new URL(`/api/v1/contributor/help/media/${encodeURIComponent(mediaId)}`, apiBaseUrl)
  const headers = new Headers()
  headers.set("Cookie", `${FOTOCORP_PLATFORM_SESSION_COOKIE}=${sessionCookie.value}`)
  headers.set("Accept", "*/*")
