...
import { approveStaffCaricature } from "@/lib/api/staff-caricatures-api"
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  if (!staffRoleCanAccessPath(staffSession.staff.role, "/staff/caricatures")) {
    return Response.json(
      { error: { code: "FORBIDDEN", message: "Your staff role cannot approve caricatures." } },
  ...
  try {
    const result = await approveStaffCaricature(assetId)
    ...
    return Response.json(
      { error: { code: "APPROVE_FAILED", message: "Could not approve caricature." } },
