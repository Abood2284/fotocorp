...
export async function POST(request: Request) {
  ...
  try {
    const data = await internalApiJson<unknown>({
      path: internalApiRoutes.adminStaffUploadWizardUploadBatches(),
      method: "POST",
      body,
    ...
    if (error instanceof InternalApiRequestError) {
      return Response.json(
        { error: { code: error.code ?? "BATCH_CREATE_FAILED", message: error.message } },
        { status: error.status >= 400 && error.status < 600 ? error.status : 502, headers: STAFF_UPLOAD_WIZARD_SAFE_HEADERS },
      )
    }
    return Response.json(
      { error: { code: "BATCH_CREATE_FAILED", message: "Could not create upload batch." } },
