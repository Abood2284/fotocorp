...
import type { JobsPipelineSnapshot } from "@/lib/api/staff-pipeline-types"
...
export async function GET(_request: NextRequest) {
  ...
  try {
    const data = await internalApiJson<JobsPipelineSnapshot>({
      path: internalApiRoutes.adminJobsPipelineSnapshot(),
      method: "GET",
    ...
    return Response.json(
      { error: { code: "UPSTREAM_ERROR", message: "Could not load pipeline snapshot." } },
