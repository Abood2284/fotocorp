...
export default async function AdminAssetDetailPage({ params, searchParams }: AdminAssetDetailPageProps) {
  ...
  async function saveEditorial(formData: FormData) {
    ...
    revalidatePath(`/staff/catalog/${id}`)
  }
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {(["thumb", "card", "detail"] as const).map((variant) => {
                  ...
                  const href = `/staff/catalog/${asset.id}?previewVariant=${variant}`
                  return (
                    <Link
                      key={variant}
                      href={href}
              ...
              {selectedPreviewVariant ? (
                <div className="overflow-hidden rounded-lg border border-border bg-muted">
                  <img
                    src={`/staff/catalog/${asset.id}/preview-image?variant=${selectedPreviewVariant}`}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {asset.r2Exists ? (
                <div className="overflow-hidden rounded-lg border border-border bg-muted">
                  <img
                    src={`/staff/catalog/${asset.id}/original-image`}
