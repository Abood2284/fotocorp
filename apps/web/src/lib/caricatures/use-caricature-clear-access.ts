...
export function useCaricatureClearAccess() {
  ...
  const isAuthenticated = Boolean(session?.kind === "user" || session?.kind === "staff")
...
async function fetchCaricatureClearAccess(): Promise<{ hasClearAccess: boolean }> {
  ...
  if (response.status === 401) return { hasClearAccess: false }
  if (!response.ok) return { hasClearAccess: false }

  const data = (await response.json().catch(() => null)) as { hasClearAccess?: boolean } | null
  return { hasClearAccess: Boolean(data?.hasClearAccess) }
}
