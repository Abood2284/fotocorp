...
const accountLinks = [
  ...
  { label: "Access", href: "/account/subscription" },
          ...
          <nav aria-label="Account navigation" className="-mx-1 flex gap-1 overflow-x-auto pb-1">
            {accountLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="whitespace-nowrap border border-border bg-background px-4 py-2 text-sm font-bold text-foreground transition-colors hover:bg-secondary/60"
              >
                {link.label}
              </Link>
            ))}
