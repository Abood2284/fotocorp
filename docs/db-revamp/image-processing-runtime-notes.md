...
- Populated automatically when **FOTOCORP publish jobs** complete (`apps/jobs` `ImagePublishProcessor` reads the canonical original buffer and upserts `image_assets_metadata` in the same transaction as go-live). The jobs Docker image **must** include workspace package `@fotocorp/original-image-metadata` (`apps/jobs/Dockerfile`); without it, publishes can still complete while leaving no metadata row. The ops scanner (`media:scan-original-metadata`) remains for legacy/missing backfill and `--retry-failed` repair.
